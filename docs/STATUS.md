...
Current phase: Deployed P0 baseline — live provider and persistent-identity integration blocked on secrets
Current objective: Supply the real name.com/Groq configuration, publish persistent identity state, pass the fully live deployed smoke test, and produce final submission media.
...
- Set only non-secret production configuration for Vercel, sandbox identity mode, Groq as the intended provider, and the deterministic refund simulator.
...
- Groq model: none configured
- `modelDriven`: false; deterministic fallback only
...
- `GROQ_API_KEY` and `GROQ_MODEL` are absent from the process and ProofRoot Vercel project. Browser credential extraction was not used because it would violate the secret-transfer boundary.
...
- Without Groq credentials/model, a deployed run cannot truthfully report `modelProvider: groq` or `modelDriven: true`.

Exact next step:
1. Supply `NAMECOM_USERNAME`, `NAMECOM_TOKEN`, `GROQ_API_KEY`, and `GROQ_MODEL` to this local Codex environment through an approved secret mechanism.
2. Run the real provider checks, provision/select a sandbox domain, generate ignored persistent identity files, and install the required secret values in Vercel production.
